...
export default function Header() {
  ...
  return (
    <>
      ...
      <div className={`bg-white border-b border-rose transition-shadow ${scrolled ? 'shadow-[0_1px_0_rgba(0,0,0,0.04)]' : ''}`}
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
         ...
         <Link to="/" className="select-none flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="flex items-center gap-4 lg:gap-5 text-ink">
              <Link to="/booking/book-appointment" className="hidden lg:inline-block text-[12px] tracking-[0.18em] uppercase border border-ink/80 px-4 py-2 hover:bg-ink hover:text-white transition-colors">Book Appointment</Link>
          ...
          <div className="max-w-[1400px] mx-auto px-10 py-10 grid grid-cols-12 gap-10">
            ...
            <div className="col-span-7 grid grid-cols-4 gap-8">
              {open && NAV.find(n => n.key === open)?.columns.map((col) => (
                <div key={col.title}>
                  <div className="text-[11px] tracking-[0.22em] uppercase text-mute mb-3">{col.title}</div>
                  <ul className="space-y-2">
                    {col.items.map((it) => (
                      <li key={it}>
                        <Link to={routeFor(open, it)} className="link-underline text-[14px] text-ink">{it}</Link>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="col-span-2">
              ...
              <div className="text-[11px] tracking-[0.22em] uppercase mt-3 text-ink">Featured · The Wedding Edit</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {mobile && (
        <div className="fixed inset-0 z-[60] bg-black/40 lg:hidden" onClick={() => setMobile(false)}>
          <div className="absolute left-0 top-0 bottom-0 w-[88%] max-w-sm bg-white p-6 overflow-y-auto" onClick={(e)=>e.stopPropagation()}>
                ...
                <li key={n.key}>
                  <details className="group border-b border-rose">
                    ...
                    <div className="pb-3 pl-2 space-y-3">
                      {n.columns.map((c) => (
                        <div key={c.title}>
                          <div className="text-[11px] tracking-[0.22em] uppercase text-mute mb-1">{c.title}</div>
                          <ul className="space-y-1">
