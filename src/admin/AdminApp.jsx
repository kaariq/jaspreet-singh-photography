...
function Shell({ children }) {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 text-neutral-900 flex">
      {/* Sidebar */}
      <aside className="w-64 bg-ink text-white flex-shrink-0 hidden lg:flex flex-col">
        ...
        <nav className="flex-1 py-4">
          {nav.map((n) => (
            <NavLink key={n.label} to={n.to} end={n.end} className={({isActive}) => `flex items-center gap-3 px-6 py-3 text-[12px] tracking-[0.18em] uppercase transition-colors ${isActive ? 'bg-white/10 text-white border-l-2 border-wine-light' : 'text-white/70 hover:text-white hover:bg-white/5 border-l-2 border-transparent'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 bg-white border-b border-neutral-200 px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="font-serif-display text-xl text-ink">Provider Console</div>
            <span className="hidden sm:inline text-[10px] tracking-[0.22em] uppercase bg-wine text-white px-2 py-0.5">{ROLES[s.activeRole]?.label}</span>
          </div>
          {/* Role / user switcher */}
          <div className="relative">
            <button onClick={() => setOpen(!open)} className="flex items-center gap-3 px-3 py-2 border border-neutral-200 hover:border-neutral-400 transition-colors">
              <span className="w-8 h-8 rounded-full bg-ink text-white flex items-center justify-center text-[11px] font-medium">{activeUser?.avatar}</span>
