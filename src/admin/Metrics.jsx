...
export default function Metrics() {
  ...
  return (
    <div className="space-y-8">
      <header>
        <h1 className="font-serif-display text-3xl text-ink">Metrics</h1>
          ...
          <ul className="space-y-3">
            {sortedCats.map(([cat, n], i) => (
              <li key={cat}>
                ...
                <div className="h-2 bg-neutral-100"><motion.div initial={{ width: 0 }} animate={{ width: `${(n / maxView) * 100}%` }} transition={{ duration: 0.7, delay: i * 0.05 }} className="h-full bg-ink"/></div>
              </li>
            ))}
          </ul>
        </Card>
        ...
        <Card title="Most-ordered categories" subtitle="From placed orders">
          <ul className="space-y-3">
            {sortedOrderCats.map(([cat, n], i) => (
              <li key={cat}>
                <div className="flex justify-between text-[12px] mb-1"><span className="capitalize font-medium">{cat}</span><span className="text-neutral-500">{n}</span></div>
                <div className="h-2 bg-neutral-100"><motion.div initial={{ width: 0 }} animate={{ width: `${(n / maxOrd) * 100}%` }} transition={{ duration: 0.7, delay: i * 0.05 }} className="h-full bg-wine"/></div>
              </li>
            ))}
          ...
          <ul className="space-y-3">
            {Object.entries(leadSources).map(([src, n], i) => (
              <li key={src} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-2 bg-neutral-100"><motion.div initial={{ width: 0 }} animate={{ width: `${(n / s.leads.length) * 100}%` }} transition={{ duration: 0.7, delay: i * 0.05 }} className="h-full bg-wine"/></div>
