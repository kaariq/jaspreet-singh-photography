    ...
    <main className="pb-12 sm:pb-20">
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 mt-6 sm:mt-12 flex sm:flex-wrap gap-2 overflow-x-auto no-scrollbar">
        {[
          ...
          <Link key={s} to={`/booking/${s}`} className={`shrink-0 text-[10px] sm:text-[11px] tracking-[0.18em] sm:tracking-[0.22em] uppercase px-3 py-1.5 sm:px-4 sm:py-2 border whitespace-nowrap transition-colors ${view===s ? 'bg-ink text-white border-ink' : 'border-rose hover:border-ink'}`}>{l}</Link>
        ))}
      </div>
    ...
    <div className="grid lg:grid-cols-12 gap-6 sm:gap-10">
      <div className="lg:col-span-4">
        <div className="lg:sticky lg:top-32">
          ...
          <p className="text-[13px] sm:text-sm text-mute mt-4 sm:mt-5 leading-relaxed">A calm, considered process — designed to keep you involved at every stage. From your first conversation to delivery, you'll have one dedicated stylist.</p>
        </div>
      </div>
      <div className="lg:col-span-8">
        <div className="relative">
          <div className="absolute left-3 sm:left-4 top-2 bottom-2 w-px bg-rose" aria-hidden="true"/>
          {PROCESS.map((p) => (
            <div key={p.n} className="relative pl-11 sm:pl-14 pb-6 sm:pb-10 last:pb-0">
              <div className="absolute left-0 top-0 w-7 h-7 sm:w-9 sm:h-9 rounded-full bg-ink text-white flex items-center justify-center font-italiana text-sm sm:text-base">{p.n}</div>
              <h3 className="font-serif-display text-xl sm:text-2xl lg:text-3xl text-ink leading-tight">{p.title}</h3>
              <p className="text-[13px] sm:text-sm text-mute mt-1.5 sm:mt-2 leading-relaxed max-w-xl">{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="grid lg:grid-cols-12 gap-6 sm:gap-10">
      ...
      <div className="lg:col-span-7">
        <h2 className="font-serif-display text-2xl sm:text-3xl lg:text-4xl text-ink">A short, simple guide.</h2>
        <p className="text-[13px] sm:text-sm text-mute mt-2 sm:mt-3 max-w-xl">Use a soft tape, stand naturally, and ask a friend to help. If you're unsure, our master tailor can come to you — free with bespoke orders.</p>
        <ol className="mt-5 sm:mt-8 space-y-4 sm:space-y-5">
          {items.map((m, i) => (
            <li key={m.label} className="flex gap-3 sm:gap-5 pb-4 sm:pb-5 border-b border-rose last:border-0">
              <div className="font-italiana text-xl sm:text-2xl text-wine w-8 sm:w-10 shrink-0">0{i+1}</div>
              <div>
  ...
  if (done) {
    return (
      <div className="max-w-2xl mx-auto bg-ink text-white p-10 lg:p-14 text-center">
        ...
        <button onClick={()=>setDone(false)} className="mt-6 inline-block border border-white/40 px-6 py-3 text-[12px] tracking-[0.22em] uppercase hover:bg-white hover:text-ink transition-colors">Book another</button>
      </div>
    );
  }
  return (
    <form onSubmit={submit} className="grid lg:grid-cols-12 gap-10">
      <div className="lg:col-span-7">
        ...
        <p className="text-sm text-mute mt-3 max-w-xl">Tell us a few details and we'll confirm within 30 minutes during studio hours.</p>
        <div className="grid sm:grid-cols-2 gap-5 mt-8">
          ...
          <div className="sm:col-span-2">
            <label className="text-[11px] tracking-[0.22em] uppercase text-mute">Notes (optional)</label>
            <textarea value={form.notes} onChange={onChange('notes')} rows="4" className="w-full mt-2 bg-white border border-rose p-3 text-sm focus:outline-none focus:border-ink"/>
          </div>
        </div>
        <button type="submit" className="mt-8 inline-flex items-center gap-2 bg-ink text-white px-6 py-3 text-[12px] tracking-[0.22em] uppercase hover:bg-wine transition-colors">Confirm appointment <ArrowRight className="w-4 h-4"/></button>
      </div>
  ...
  return (
    <div className="grid lg:grid-cols-12 gap-10">
      <div className="lg:col-span-5">
        ...
        <h2 className="font-serif-display text-4xl lg:text-5xl mt-3 text-ink">Book your free virtual consultation.</h2>
        <p className="text-sm text-mute mt-4">A relaxed video call with our e-designer. We'll discuss your inspirations, recommend fabrics, and share a written quote within the hour.</p>
        <ul className="mt-6 space-y-2 text-sm">
          {['Free, no obligation','Sketches & swatches included','Pricing emailed within 1 hour'].map(x=> <li key={x} className="flex items-center gap-2"><Check className="w-4 h-4 text-wine"/>{x}</li>)}
        </ul>
      </div>
      <div className="lg:col-span-7 bg-blush border border-rose p-6 lg:p-8">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
          {days.map((d, i) => (
            <button key={i} onClick={()=>setSel(i)} className={`shrink-0 w-16 py-3 border text-center transition-colors ${sel===i ? 'bg-ink text-white border-ink' : 'border-rose hover:border-ink'}`}>
        ...
        <div className="text-[11px] tracking-[0.22em] uppercase text-mute mt-6 mb-3">Choose a time (IST)</div>
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
          {['11:00','12:00','13:00','14:30','15:30','16:30','18:00','19:00'].map(t => (
            <button key={t} onClick={()=>setTime(t)} className={`py-2 border text-sm transition-colors ${time===t ? 'bg-ink text-white border-ink' : 'border-rose hover:border-ink'}`}>{t}</button>
          ))}
        </div>
        <button onClick={()=>setDone(true)} className="mt-8 inline-flex items-center gap-2 bg-ink text-white px-6 py-3 text-[12px] tracking-[0.22em] uppercase hover:bg-wine transition-colors">Confirm slot <ArrowRight className="w-4 h-4"/></button>
      </div>
    </div>
  );
}
...
function Field({ label, cls='', ...props }) {
  return (
    <div className={cls}>
      ...
      <input {...props} className="w-full mt-2 bg-white border border-rose p-3 text-sm focus:outline-none focus:border-ink"/>
    </div>
  );
}
function Select({ label, options, ...props }) {
  return (
    <div>
      <label className="text-[11px] tracking-[0.22em] uppercase text-mute">{label}</label>
      <select {...props} className="w-full mt-2 bg-white border border-rose p-3 text-sm focus:outline-none focus:border-ink">
...
function FAQSection() {
  return (
    <section className="max-w-[1100px] mx-auto px-4 sm:px-6 lg:px-10 mt-12 sm:mt-20">
      ...
      <h2 className="font-serif-display text-3xl sm:text-4xl lg:text-5xl text-ink text-center mt-2 sm:mt-3">Good to know.</h2>
        ...
        {FAQ.map((f) => (
          <details key={f.q} className="group py-4 sm:py-5">
            <summary className="flex items-center justify-between gap-3 cursor-pointer list-none">
              <span className="font-serif-display text-lg sm:text-xl lg:text-2xl text-ink leading-tight">{f.q}</span>
              <span className="font-italiana text-2xl text-wine group-open:rotate-45 transition-transform shrink-0">+</span>
            </summary>
