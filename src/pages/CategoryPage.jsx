    ...
    <main className="pb-12 sm:pb-20">
      ...
      <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 mt-8 sm:mt-12">
        <div className="flex items-end justify-between gap-3 mb-4 sm:mb-6">
          <div>
            <div className="edit-num text-[#6B4046]">—  PREDEFINED DESIGNS</div>
            <h2 className="font-serif-display text-2xl sm:text-3xl lg:text-4xl mt-1.5 sm:mt-2">Pick a starting silhouette.</h2>
          </div>
          <span className="text-[10px] sm:text-[11px] tracking-[0.18em] sm:tracking-[0.22em] uppercase text-[#6B4046] whitespace-nowrap">{designs.length} designs</span>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {designs.map((d, i) => (
            <motion.button
              ...
              className="group text-left bg-white border border-[#E0BCC0] hover:border-[#2A0A12] transition-colors"
            >
              <div className="relative aspect-[3/4] overflow-hidden bg-[#ECD4D7]">
              ...
              <div className="p-3 sm:p-4">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-serif-display text-base sm:text-xl text-[#2A0A12] leading-tight">{d.label}</h3>
                  <ArrowUpRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 mt-0.5 sm:mt-1 text-[#6B4046] group-hover:text-[#6E0D25] group-hover:-translate-y-0.5 group-hover:translate-x-0.5 transition-all shrink-0"/>
                </div>
                <p className="text-[11px] sm:text-[13px] text-[#6B4046] mt-1 leading-relaxed line-clamp-2">{d.blurb}</p>
                <div className="flex items-center justify-between mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-[#E0BCC0]">
                  <span className="text-[9px] sm:text-[11px] tracking-[0.18em] sm:tracking-[0.22em] uppercase text-[#6B4046]">From</span>
                  <span className="font-serif-display text-sm sm:text-lg text-[#2A0A12]">₹{d.basePrice.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </motion.button>
          ))}
        ...
        <div className="mt-8 sm:mt-12 grid lg:grid-cols-2 gap-4 sm:gap-6">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="relative overflow-hidden bg-[#2A0A12] text-white p-6 sm:p-8 lg:p-12">
            <Sparkles className="w-6 h-6 sm:w-7 sm:h-7 text-[#8B1A30]"/>
            ...
            <Link to={`/tailoring/${slug}/order/custom`} className="inline-flex items-center gap-2 mt-4 sm:mt-6 bg-white text-[#2A0A12] px-4 sm:px-5 py-2.5 sm:py-3 text-[10px] sm:text-[11px] tracking-[0.18em] sm:tracking-[0.22em] uppercase hover:bg-[#6E0D25] hover:text-white transition-colors"><Upload className="w-4 h-4"/>Upload your design</Link>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.1 }} className="relative overflow-hidden bg-[#F7EBEC] border border-[#E0BCC0] p-6 sm:p-8 lg:p-12">
            <div className="edit-num text-[#6E0D25]">—  NOT SURE WHAT SUITS YOU?</div>
            <h3 className="font-serif-display text-2xl sm:text-3xl mt-2 sm:mt-3">Talk to a designer.</h3>
            <p className="text-[13px] sm:text-sm text-[#6B4046] mt-2 sm:mt-3 max-w-md">Free 30-minute video call. Share your inspirations, body type and budget — we’ll suggest the right silhouette.</p>
            <Link to="/booking/virtual-consultation" className="inline-flex items-center gap-2 mt-4 sm:mt-6 bg-[#2A0A12] text-white px-4 sm:px-5 py-2.5 sm:py-3 text-[10px] sm:text-[11px] tracking-[0.18em] sm:tracking-[0.22em] uppercase hover:bg-[#6E0D25] transition-colors">Book consultation <ArrowRight className="w-4 h-4"/></Link>
          </motion.div>
        </div>
      </section>
    </main>
