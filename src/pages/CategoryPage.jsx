    ...
    <main className="pb-12 sm:pb-20">
      ...
      <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 mt-8 sm:mt-12">
        <div className="flex items-end justify-between gap-3 mb-4 sm:mb-6">
          ...
          <span className="text-[10px] sm:text-[11px] tracking-[0.18em] sm:tracking-[0.22em] uppercase text-mute whitespace-nowrap">{designs.length} designs</span>
        </div>
            ...
            >
              ...
              <div className="p-3 sm:p-4">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-serif-display text-base sm:text-xl text-ink leading-tight">{d.label}</h3>
                ...
                <p className="text-[11px] sm:text-[13px] text-mute mt-1 leading-relaxed line-clamp-2">{d.blurb}</p>
                <div className="flex items-center justify-between mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-rose">
                  <span className="text-[9px] sm:text-[11px] tracking-[0.18em] sm:tracking-[0.22em] uppercase text-mute">From</span>
                  <span className="font-serif-display text-sm sm:text-lg text-ink">₹{d.basePrice.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </motion.button>
          ))}
        ...
        <div className="mt-8 sm:mt-12 grid lg:grid-cols-2 gap-4 sm:gap-6">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="relative overflow-hidden bg-ink text-white p-6 sm:p-8 lg:p-12">
            ...
            <Link to={`/tailoring/${slug}/order/custom`} className="inline-flex items-center gap-2 mt-4 sm:mt-6 bg-white text-ink px-4 sm:px-5 py-2.5 sm:py-3 text-[10px] sm:text-[11px] tracking-[0.18em] sm:tracking-[0.22em] uppercase hover:bg-wine hover:text-white transition-colors"><Upload className="w-4 h-4"/>Upload your design</Link>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.1 }} className="relative overflow-hidden bg-blush border border-rose p-6 sm:p-8 lg:p-12">
            ...
            <p className="text-[13px] sm:text-sm text-mute mt-2 sm:mt-3 max-w-md">Free 30-minute video call. Share your inspirations, body type and budget — we’ll suggest the right silhouette.</p>
            <Link to="/booking/virtual-consultation" className="inline-flex items-center gap-2 mt-4 sm:mt-6 bg-ink text-white px-4 sm:px-5 py-2.5 sm:py-3 text-[10px] sm:text-[11px] tracking-[0.18em] sm:tracking-[0.22em] uppercase hover:bg-wine transition-colors">Book consultation <ArrowRight className="w-4 h-4"/></Link>
          </motion.div>
        </div>
      </section>
    </main>
