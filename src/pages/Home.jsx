...
function HeroCarousel() {
  ...
  return (
    <section className="relative w-full bg-ink overflow-hidden" onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <motion.div variants={blurUp} className={`flex flex-wrap gap-2 sm:gap-3 mt-5 sm:mt-8 ${alignLeft ? '' : 'lg:justify-end'}`}>
                ...
                <Link to="/booking/book-appointment" className="inline-flex items-center gap-2 border border-white/70 px-4 sm:px-7 py-2.5 sm:py-3.5 text-[10.5px] sm:text-[12px] tracking-[0.18em] sm:tracking-[0.22em] uppercase text-white hover:bg-white hover:text-ink transition-colors">
                  Book Appointment
                </Link>
              </motion.div>
            </motion.div>
    ...
    <div className="border-y border-rose bg-white overflow-hidden">
      <div className="flex marquee-track whitespace-nowrap py-4">
        {[...items, ...items, ...items].map((t, i) => (
          <span key={i} className="inline-flex items-center gap-3 px-8 text-[12px] tracking-[0.28em] uppercase text-ink">
            <span className="font-italiana text-base text-wine">—</span>{t}
          </span>
        ))}
      </div>
    </div>
...
function ServicesGrid() {
  return (
    <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-12 sm:py-20 lg:py-32">
      ...
      <Reveal variants={stagger(0.05)} className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-rose border border-rose">
        {SERVICES.map((s, i) => (
    ...
    <section ref={ref} className="bg-ink text-white py-12 sm:py-20 lg:py-32 overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10">
        ...
        <div className="relative">
          ...
          <motion.div className="hidden md:block absolute left-0 top-[42px] h-px bg-wine" style={{ width: lineW }} aria-hidden="true"/>
              ...
              <motion.div key={p.n} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={{ duration: 0.8, delay: idx * 0.12, type: 'spring', stiffness: 80, damping: 16 }} className="relative group">
                <motion.div whileHover={{ y: -6 }} transition={{ type: 'spring', stiffness: 250, damping: 20 }} className="flex flex-col items-start">
                  <div className="relative z-10 w-[84px] h-[84px] rounded-full bg-ink border-2 border-white/25 group-hover:border-wine flex items-center justify-center transition-colors">
                    <span className="font-italiana text-3xl text-rose-soft group-hover:text-wine transition-colors">{p.n}</span>
                  </div>
        ...
        <Reveal className="mt-10 sm:mt-16 lg:mt-20">
          <Link to="/booking/our-process" className="inline-flex items-center gap-2 border border-white/40 px-5 sm:px-6 py-2.5 sm:py-3 text-[11px] sm:text-[12px] tracking-[0.22em] uppercase hover:bg-white hover:text-ink transition-colors">Read full process <ArrowRight className="w-4 h-4"/></Link>
        </Reveal>
      </div>
    </section>
  )
    ...
    <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-12 sm:py-20 lg:py-32">
      ...
      <div className="lg:hidden -mx-4 px-4 flex gap-3 overflow-x-auto snap-x snap-mandatory no-scrollbar pb-2">
        {COLLECTIONS.map((c, i) => (
          <motion.div key={c.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: i * 0.06, ease }} className="snap-start flex-shrink-0 w-[58%] sm:w-[44%]">
            <Link to={`/collections/${c.tag.toLowerCase().replace(/\s+/g, '-')}`} className="group relative overflow-hidden bg-rose-pale block aspect-[3/4]">
      ...
      <div className="hidden lg:grid lg:grid-cols-12 gap-6 lg:gap-8">
        {COLLECTIONS.map((c, i) => (
          ...
          >
            <Link to={`/collections/${c.tag.toLowerCase().replace(/\s+/g, '-')}`} className="group relative overflow-hidden bg-rose-pale block aspect-[4/3]">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-ink-deep/72 via-ink-deep/15 to-transparent"/>
      ...
      <Reveal variants={stagger(0.1)} className="relative h-full flex items-center justify-center text-center px-4 sm:px-6">
        <div className="max-w-3xl text-white">
          ...
          <motion.div variants={blurUp}>
            <Link to="/explore/gallery-and-lookbook" className="inline-flex items-center gap-2 mt-5 sm:mt-8 border border-white px-5 sm:px-6 py-2.5 sm:py-3 text-[11px] sm:text-[12px] tracking-[0.22em] uppercase hover:bg-white hover:text-ink transition-colors">View the atelier <ArrowRight className="w-4 h-4"/></Link>
          </motion.div>
        </div>
      </Reveal>
    </section>
    ...
    <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-12 sm:py-20 lg:py-32">
      ...
      <Reveal variants={stagger(0.04)} className="grid grid-cols-3 lg:grid-cols-3 gap-px bg-rose border border-rose">
        {FEATURES.slice(0, 6).map((f, i) => {
          ...
          return (
            <motion.div key={f.title} custom={i} variants={dropIn} className="bg-white p-3 sm:p-6 lg:p-8 hover:bg-blush transition-colors group">
              <Icon className="w-4 h-4 sm:w-7 sm:h-7 text-wine group-hover:scale-110 transition-transform" strokeWidth={1.4}/>
              <h3 className="font-serif-display text-[12px] sm:text-xl mt-2 sm:mt-5 text-ink leading-tight">{f.title}</h3>
              <p className="hidden sm:block text-[12px] sm:text-sm text-mute mt-1.5 sm:mt-2 leading-relaxed">{f.desc}</p>
            </motion.div>
          );
        })}
      </Reveal>
            ...
            {list.map((t, i) => (
              ...
              >
                <div className="relative w-[96px] sm:w-[160px] lg:w-[180px] aspect-[3/4] overflow-hidden flex-shrink-0 bg-black">
                  ...
                  <span className="absolute top-2 left-2 sm:top-3 sm:left-3 text-[8px] sm:text-[9px] tracking-[0.18em] sm:tracking-[0.22em] uppercase bg-white/95 text-ink px-1.5 py-0.5">{t.type === 'video' ? 'Video' : 'Photo'}</span>
        ...
        {open && (
          <motion.div className="fixed inset-0 z-[80] bg-black/85 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setOpen(null)}>
            <motion.div className="relative max-w-4xl w-full" initial={{ scale: 0.94, y: 24, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ scale: 0.94, y: 24, opacity: 0 }} transition={{ type: 'spring', stiffness: 220, damping: 24 }} onClick={(e) => e.stopPropagation()}>
              <button onClick={() => setOpen(null)} aria-label="Close" className="absolute -top-12 right-0 w-10 h-10 rounded-full bg-white/15 hover:bg-white text-white hover:text-ink flex items-center justify-center"><X className="w-5 h-5"/></button>
              ...
              <div className="bg-white p-6">
                ...
                <div className="mt-3">
                  ...
                  <div className="text-[11px] tracking-[0.22em] uppercase text-mute">{open.role}</div>
                </div>
              </div>
            </motion.div>
          </motion.div>
  ...
  return (
    <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-10 sm:py-20 lg:py-32">
      ...
      <Reveal variants={stagger(0.04)} className="lg:hidden border-t border-rose">
        {BLOG.slice(0, 3).map((b, i) => (
          <motion.article key={b.title} custom={i} variants={fadeUp}>
            <Link to="/explore/blog-and-fashion-news" className="group flex items-center gap-3 py-2.5 border-b border-rose">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[8.5px] tracking-[0.2em] uppercase text-mute leading-none">{b.tag} · {b.date}</div>
                <h3 className="font-serif-display text-[13px] text-ink leading-snug line-clamp-1 mt-0.5">{b.title}</h3>
              </div>
      ...
      <Reveal variants={stagger(0.04)} className="hidden lg:block border-t border-rose">
        {BLOG.map((b, i) => (
          <motion.article key={b.title} custom={i} variants={fadeUp}>
            <Link to="/explore/blog-and-fashion-news" className="group grid grid-cols-12 gap-6 items-center py-7 lg:py-8 border-b border-rose hover:bg-blush transition-colors px-2 lg:px-3">
              <div className="col-span-1 font-italiana text-2xl lg:text-3xl text-wine">0{i + 1}</div>
              <div className="col-span-3 text-[11px] tracking-[0.22em] uppercase text-mute">{b.tag} · {b.date}</div>
              <h3 className="col-span-6 font-serif-display text-2xl lg:text-3xl text-ink leading-snug group-hover:text-wine transition-colors line-clamp-2">{b.title}</h3>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-10 sm:py-20 lg:py-28">
        <Reveal variants={stagger(0.05)} className="grid lg:grid-cols-12 gap-6 lg:gap-12 items-start">
          <motion.div variants={fadeUp} className="lg:col-span-4 lg:sticky lg:top-32">
            ...
            <p className="hidden lg:block text-sm text-mute mt-5 leading-relaxed max-w-sm">Still curious? Our concierge is on WhatsApp every day, 10am–8pm IST.</p>
          </motion.div>
              ...
              return (
                <div key={f.q} className="border-b border-rose-soft">
                  <button onClick={() => setOpen(isOpen ? -1 : i)} className="w-full text-left flex items-start gap-3 py-3.5 sm:py-5 group">
                    <span className="font-italiana text-wine text-sm sm:text-lg pt-0.5 sm:pt-1 w-6 sm:w-8 shrink-0">0{i + 1}</span>
                    <span className="flex-1 font-serif-display text-[14px] sm:text-[20px] lg:text-[22px] text-ink leading-snug pr-3">{f.q}</span>
                    <span className={`text-ink text-xl sm:text-2xl leading-none transition-transform duration-300 ${isOpen ? 'rotate-45' : ''}`}>+</span>
                  </button>
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.35, ease }} className="overflow-hidden">
                        <p className="text-[12.5px] sm:text-[14px] text-mute leading-relaxed pl-9 sm:pl-11 pr-6 pb-4 sm:pb-5">{f.a}</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
