...
export default function RequestContact() {
  ...
  const designObj = isCustom
    ? { id: 'custom', label: 'Custom Design', basePrice: 0, image: designs[0]?.image, blurb: 'Tell us your vision — our designers will craft it.' }
  ...
  if (!designObj) {
    return (
      <main className="min-h-[60vh] flex items-center justify-center">
        <p className="text-mute">Design not found. <Link to="/tailoring" className="underline">Browse tailoring</Link></p>
      </main>
    );
  }
  ...
  return (
    <main className="pb-20 bg-blush min-h-screen">
      <Breadcrumb items={[
        ...
        { label: CATEGORY_LABELS[schemaKey] || slug, to: `/tailoring/${slug}` },
        { label: designObj.label },
      ]} />
        ...
        <div className="grid lg:grid-cols-[400px_1fr] gap-6 lg:gap-10">
          ...
          <div className="border border-rose bg-white">
            <div className="aspect-[4/5] bg-rose-pale overflow-hidden">
              <img src={designObj.image} alt={designObj.label} className="w-full h-full object-cover"/>
            </div>
            <div className="p-5">
              ...
              <div className="flex items-center justify-between mt-5 pt-4 border-t border-rose">
                <span className="text-[10px] tracking-[0.22em] uppercase text-mute">Starting at</span>
            ...
            {!done ? (
              <>
                ...
                <p className="text-[14px] text-mute mt-3 leading-relaxed max-w-lg">
                  Our design team will call you to discuss measurements, fabric, customisations and pricing.
                  You can also queue more designs into your cart and we’ll discuss them all together.
                </p>

                <div className="mt-6 space-y-4 max-w-lg">
                  <div>
                    <label className="block text-[10px] tracking-[0.22em] uppercase text-mute mb-1">Full name</label>
                    <input
                  ...
                  <div>
                    <label className="block text-[10px] tracking-[0.22em] uppercase text-mute mb-1">Phone number</label>
                    <input
                  ...
                  <div>
                    <label className="block text-[10px] tracking-[0.22em] uppercase text-mute mb-1">Notes (optional)</label>
                    <textarea
                ...
                {cart.length > 0 && (
                  <div className="mt-6 text-[12px] text-mute">
                    You already have <span className="text-ink font-medium">{cart.length}</span> design{cart.length>1?'s':''} in your cart.{' '}
                    <Link to="/cart" className="underline text-wine">View cart</Link>
                  </div>
                )}
              </>
            ) : (
              <div className="py-8">
                <div className="w-12 h-12 rounded-full bg-wine text-white flex items-center justify-center"><Check className="w-6 h-6"/></div>
                ...
                <p className="text-[14px] text-mute mt-2 max-w-lg leading-relaxed">
                  Thanks {name.split(' ')[0]} — our team will call you on <span className="text-ink">{phone}</span> within 24 hours
                  to walk you through your <span className="text-ink">{designObj.label}</span> order.
                </p>
                <div className="mt-6 flex flex-col sm:flex-row gap-3">
                  <Link to="/cart" className="inline-flex items-center gap-2 bg-wine text-white px-6 py-3 text-[11px] tracking-[0.22em] uppercase hover:bg-wine-light"><ShoppingBag className="w-4 h-4"/>View cart</Link>
                  <Link to="/tailoring" className="inline-flex items-center gap-2 border border-ink text-ink px-6 py-3 text-[11px] tracking-[0.22em] uppercase hover:bg-ink hover:text-white"><ArrowLeft className="w-4 h-4"/>Browse more designs</Link>
                </div>
              </div>
            )}
          </div>
