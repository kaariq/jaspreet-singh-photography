    ...
    <main className="min-h-[80vh] grid lg:grid-cols-2 bg-white">
      ...
      <div className="flex items-center justify-center p-8 lg:p-16">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} className="w-full max-w-md">
          ...
          <h1 className="font-serif-display text-4xl mt-3 text-ink">{mode === 'login' ? 'Sign in' : 'Create account'}</h1>
          <p className="text-sm text-mute mt-2">{mode === 'login' ? 'Welcome back. Sign in to manage measurements, orders and appointments.' : 'A free account stores your measurements, orders and appointments forever.'}</p>

          <div className="flex gap-1 mt-6 p-1 bg-blush border border-rose w-fit">
            {['login', 'register'].map((m) => (
              <button key={m} onClick={() => setMode(m)} className={`px-5 py-2 text-[11px] tracking-[0.22em] uppercase transition-colors ${mode === m ? 'bg-ink text-white' : 'text-ink hover:bg-white'}`}>{m === 'login' ? 'Sign in' : 'Create account'}</button>
            ))}
          </div>
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            ...
            <button type="submit" className="group w-full inline-flex items-center justify-center gap-2 bg-ink text-white px-7 py-3.5 text-[12px] tracking-[0.22em] uppercase hover:bg-wine transition-colors">
              {mode === 'login' ? 'Sign in' : 'Create account'} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform"/>
            </button>
            <p className="text-[11px] tracking-[0.18em] uppercase text-mute text-center">By continuing you agree to our terms & privacy.</p>
          </form>
        </motion.div>
      </div>
    </main>
  ...
  return (
    <label className="block">
      ...
      <div className="flex items-center gap-2 mt-1.5 bg-white border border-rose focus-within:border-ink transition-colors">
        <span className="pl-3 text-mute"><Icon className="w-4 h-4"/></span>
